...
export default function CustomerKhataTab() {
  ...
  const fetchKhata = React.useCallback(async () => {
    if (!targetCustomerId) return;

    setIsFetching(true);
    setKhata(null);
    try {
      const res = await fetch(`/accounting/customers/${targetCustomerId}/aging`, { headers: { ...authHdr(), 'Content-Type': 'application/json' } });
      if (res.ok) {
        ...
        if (!data.customer_name) {
          try {
            const custRes = await fetch(`/customer/${targetCustomerId}`, { headers: { ...authHdr(), 'Content-Type': 'application/json' } });
            if (custRes.ok) {
              ...
              if (custData && custData.name) {
                ...
                data = {
                  ...data,
                  customer_id: custData.id || targetCustomerId,
    ...
    <div className="space-y-6">
      {isFetching && !khata && (
        <div className="p-12 text-center text-slate-400 flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-bold uppercase tracking-wider">Loading Khata...</p>
        </div>
      )}
