...
function VehicleInventoryTab() {
  ...
  const fetchInventory = async (currentPage, currentSearch, currentVehicle, currentProduct) => {
    setIsLoading(true);
    try {
      let url = `/inventory/vehicle-inventory?page=${currentPage}&limit=${limit}`;
      if (currentSearch) url += `&search=${encodeURIComponent(currentSearch)}`;
      if (currentVehicle) url += `&vehicle_id=${currentVehicle}`;
      if (currentProduct) url += `&product_id=${currentProduct}`;
    ...
    <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-slate-800">Vehicle Inventory</h2>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
          <select 
            value={vehicleId} 
          ...
          >
            ...
            {vehicles.map(v => <option key={v.id || v._id} value={v.id || v._id}>{v.vehicle_number} {v.vehicle_type ? `- ${v.vehicle_type}` : ''}</option>)}
          </select>
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200">
            <tr>
              ...
              <th className="p-4 text-right text-emerald-600">Transferred In</th>
              <th className="p-4 text-right text-sky-600">Sold</th>
              <th className="p-4 text-right font-black text-indigo-700 bg-indigo-50/50">Available</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
            {isLoading ? (
               <tr><td colSpan="5" className="p-10 text-center text-slate-500 font-bold">Loading...</td></tr>
            ) : inventory.length === 0 ? (
               <tr><td colSpan="5" className="p-10 text-center text-slate-500 font-bold">No stock available</td></tr>
            ) : (
               inventory.map((item, idx) => (
                 <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                   <td className="p-4">
                     <div className="font-bold text-slate-900 text-sm">{item.product_name || 'N/A'}</div>
                     <div className="text-xs text-slate-500 mt-0.5">{item.variant_name || 'N/A'} {item.sku ? `(${item.sku})` : ''}</div>
                     <div className="text-[10px] text-slate-400 mt-0.5">{item.variant_qty} {item.unit} = 1 {item.package}</div>
                   </td>
                   <td className="p-4">
                     <div className="font-semibold text-slate-800">{item.vehicle?.vehicle_number || item.vehicle_number || 'N/A'}</div>
                     <div className="text-xs text-slate-500">{item.vehicle?.model || ''} {item.vehicle?.vehicle_type ? `(${item.vehicle.vehicle_type})` : ''}</div>
                   </td>
                   <td className="p-4 text-right font-bold text-emerald-600">{item.warehouse_to_vehicle_quantity || 0}</td>
                   <td className="p-4 text-right font-bold text-sky-600">{item.sale_quantity || 0}</td>
                   <td className="p-4 text-right font-black text-indigo-700 text-sm bg-indigo-50/30">{item.available_quantity || 0}</td>
                 </tr>
               ))
            )}
          </tbody>
