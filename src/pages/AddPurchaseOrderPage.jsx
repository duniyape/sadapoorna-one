          ...
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 flex flex-col justify-end">
              <div className="flex justify-between items-center text-slate-600 mb-2">
                ...
                <span className="font-bold text-sm">₹{formData.items.reduce((sum, item) => {
                  const itemVariants = variants[item.product_id] || [];
                  const qtyPerPkg = parseFloat(itemVariants.find(v => (v.id || v._id) === item.variant_id)?.quantity_per_package) || 1;
                  const rateType = itemVariants.find(v => (v.id || v._id) === item.variant_id)?.rate_type || 'per_package';
                  const multiplier = rateType === 'per_unit' ? qtyPerPkg : 1;
              ...
              <div className="flex justify-between items-center text-slate-900 border-t border-slate-200 pt-3 mt-1">
                ...
                <span className="font-black text-2xl text-emerald-600">
                  ₹{(formData.items.reduce((sum, item) => {
                    ...
                    const multiplier = rateType === 'per_unit' ? qtyPerPkg : 1;
                    return sum + ((parseFloat(item.quantity) || 0) * multiplier * (parseFloat(item.rate) || 0));
                  }, 0) - (parseFloat(formData.discount) || 0) + (parseFloat(formData.other_charges) || 0)).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </span>
              </div>
            </div>
          </div>
